...
class Vidos extends Admin
{
    ...
    public function index(){
        ...
        $html= builder('list')
            ...
            ->addRightButton('edit')
            ->addRightButton('delete',array('title' => '删除视频', 'confirm-info' => '你确定要删除视频吗？', 'href' => url(MODULE_NAME . '/' . CONTROLLER_NAME . '/delete', ['id' => '__data_id__'])))
    ...
    public function edit($id = 0) {
        ...
        if (IS_POST) {
            ...
            if (empty($data['title'])){
                $this->error('视频标题不能为空');exit();
            }
            if (empty($data['void'])){
                $this->error('视频不能为空');exit();
            }
            if (empty($data['void_time'])){
                $this->error('视频时长不能为空');exit();
            }
            $data['type']=3;//视频展示
            ...
            if ($id !=0){//编辑
                $vidos=db('article')->where('id',$id)->find();
                $vidos['void_path']=get_icon($vidos['void']);
                $this->view->assign('vidos',$vidos);
                return $this->fetch();
            }else{//新增
