 ...
 * 问题分类
 */
class ProblemType extends Admin
{
    protected $problemTypeModel;
    //protected $groupIds;
    ...
    function _initialize()
    {
        ...
        $this->problemTypeModel = model('ProblemType');
    }
    ...
    public function index()
    {
        ...
        return (new Iframe())
          ...
          ->search([
                ...
                ['name' => 'title', 'type' => 'text', 'extra_attr' => 'placeholder="请输入类型名称"'],
            ])
            ->content($this->grid());
    }
    ...
    public function grid()
    {
        ...
        list($data_list, $total) = $this->problemTypeModel->search($search_setting)->getListByPage($condition, true, 'create_time asc');
      ...
      /*  $reset_password = [
        ...
        return builder('list')
            ...
            ->addRightButton('edit')
            ->addRightButton('delete',array('title' => '删除类型', 'confirm-info' => '你确定要删除类型吗？', 'href' => url(MODULE_NAME . '/' . CONTROLLER_NAME . '/delete', ['id' => '__data_id__'])))
    ...
    public function edit($id = 0) {
        ...
        if (IS_POST) {
            ...
            $result = $this->problemTypeModel->editData($data);
            if ($result) {
                $this->success($title.'成功', url('index'));
            } else {
                $this->error($this->problemTypeModel->getError());
            }
        } else {
            // 获取账号信息
            if ($id>0) {
                $info = $this->problemTypeModel->get($id);
            }else{
                $info=[];
            }
            $builder = builder('Form')
                ->addFormItem('id', 'hidden', 'UID', '')
                ->addFormItem('title', 'text', '类型名称', '填写一个有个性的名称吧','','require')
    ...
    public function status(){
        ...
        //更改状态为可修改
        $rest = Db::table('eacoo_offer')->where($where)->update(array('status'=>2));
        if (false !=$rest){
            $this->success('更改成功');
        }else{
            $this->error('更改失败');
    ...
    public function ntatus(){
        ...
        $where['id']=array('in',$ids);
        //更改状态为可修改
        $rest = Db::table('eacoo_offer')->where($where)->update(array('status'=>3));
        if (false !=$rest){
            $this->success('更改成功');
        }else{
            $this->error('更改失败');
    ...
    public function delete($id=0){
        ...
        //删除类型
        $rest=Db::table('eacoo_problem_type')->where('id',$id)->delete();
        if (false !=$rest){
            $this->success('删除成功');
        }else{
            $this->error('删除失败');
