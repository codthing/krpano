...
class Ad extends Admin
{
    ...
    public function index()
    {
       ...
       // $type=db('category')->where('type',1)->where('status',1)->column('id,title');
        return (new Iframe())
            ->setMetaTitle('广告列表')
             /*->search([
                ['name' => 'position_id', 'type' => 'select', 'title' => '广告位置', 'options' => $type],
    ...
    public function grid()
    {
        ...
        list($data_list, $total) = $this->adModel->search($search_setting)->getListByPage($condition, true, 'create_time asc');
        ...
        return builder('list')
            ...
            ->keyListItem('icon', '广告图','picture',['width'=>30])
            ...
            ->addRightButton('edit')
            ->addRightButton('delete', array('title' => '删除banner', 'confirm-info' => '你确定要删除banner吗？', 'href' => url(MODULE_NAME . '/' . CONTROLLER_NAME . '/delete', ['id' => '__data_id__'])))
    ...
    public function edit($id = 0)
    {
        ...
        if (IS_POST) {
            ...
            $result = $this->adModel->editData($data);
            if ($result) {
    ...
    public function form($id = 0)
    {
        ...
        $form = builder('Form')
            ...
            ->addFormItem('icon', 'picture', 'banner图')
    ...
    public function delete($id = 0)
    {
        ...
        //删除问题
        $rest = Db::table('eacoo_ad')->where('id', $id)->delete();
        if (false != $rest) {
            $this->success('删除成功');
        } else {
            $this->error('删除失败');
